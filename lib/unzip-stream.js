...
const states = {
    ...
    FILE_DATA: 3,
    DATA_DESCRIPTOR: 4,
    CENTRAL_DIRECTORY_FILE_HEADER: 5,
    CENTRAL_DIRECTORY_FILE_HEADER_SUFFIX: 6,
    CENTRAL_DIRECTORY_END: 7,
    CENTRAL_DIRECTORY_END_COMMENT: 8,
...
UnzipStream.prototype.processDataChunk = function (chunk) {
    ...
    switch (this.state) {
        ...
        case states.LOCAL_FILE_HEADER_SUFFIX:
            requiredLength = this.currentEntryData.fileNameLength + this.currentEntryData.extraFieldLength;
            break;
        ...
        case states.CENTRAL_DIRECTORY_FILE_HEADER_SUFFIX:
            requiredLength = this.currentEntryData.fileNameLength + this.currentEntryData.extraFieldLength + this.currentEntryData.fileCommentLength;
            break;
    ...
    switch (this.state) {
        ...
        case states.LOCAL_FILE_HEADER:
            this.currentEntryData = this._readFile(chunk);
            this.state = states.LOCAL_FILE_HEADER_SUFFIX;

            return requiredLength;

        case states.LOCAL_FILE_HEADER_SUFFIX:
            var fileName = chunk.slice(0, this.currentEntryData.fileNameLength).toString();
            var entry = new stream.PassThrough();
            entry.path = fileName;
            this._prepareOutStream(this.currentEntryData, entry);
        ...
        case states.CENTRAL_DIRECTORY_FILE_HEADER:
            this.currentEntryData = this._readCentralDirectoryEntry(chunk);
