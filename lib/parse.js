...
var MatchStream = require('./matcher-stream');
      ...
      self._pullStream.pull(vars.extraFieldLength, function (err, extraField) {
        ...
        if (vars.compressionMethod === 0) {
          ...
          if (fileSizeKnown) {
            ...
            pattern.writeUInt32LE(0x08074b50, 0);
            var searchPattern = {
              pattern: pattern,
              requiredExtraSize: 12
            }

            var matchStream = new MatchStream(searchPattern, function (matchedChunk, sizeSoFar) {
              var vars = binary.parse(matchedChunk)
              ...
              self._pullStream.prepend(matchedChunk);
